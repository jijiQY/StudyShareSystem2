...
<body>
		...
		<section class="Hui-article-box">
			<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 评论管理
			...
			<div class="mt-20">
				...
				<table class="table table-border table-bordered table-hover table-bg table-sort"  >
					...
					<tbody>
						...
						<c:forEach items="${page.content }" var="commentManager">
				<tr class="text-c">
					<td >${commentManager.id }</td>
					<td >${commentManager.user.nickName }</td>
					<td>
						<fmt:formatDate value="${commentManager.commentTime }" pattern="yyyy-MM-dd hh:mm:ss"/>
					</td>
					<td>${commentManager.content }</td>
					<td >${commentManager.article.title }</td>
					<td>
					<button class="btn radius btn-primary size-S" onclick="lookover('${commentManager.id }')">查看</button>
								<button class="btn radius btn-primary size-S" onclick="del(this,'${commentManager.id }')">删除</button>
					</td>
				</tr>
			</c:forEach>
			<tr>
